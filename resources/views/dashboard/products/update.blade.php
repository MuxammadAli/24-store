...
@section('content')
    <div id="app">
        <product-edit :product="{{ json_encode($product) }}" :brands="{{json_encode($brands) }}" :categories="{{ json_encode($categories) }}" :colors="{{ json_encode($colors) }}" :units="{{ json_encode($units) }}"></product-edit>
    </div>
@endsection
